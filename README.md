...
### LOCAL LLM 기반 수동 제어
Ollama를 통해 로컬에서 구동되는 Qwen 2.5 3B 모델을 활용하여 로봇의 기초 이동을 제어
- 자연어 명령 해석: 사용자의 채팅 명령(예: "앞으로 1m", "오른쪽으로 돌아")을 분석하여 로봇이 이해할 수 있는 이동 시퀀스로 변환
- 저지연 온디바이스 추론: 외부 클라우드 연결 없이 로컬 한정적인 GPU 자원을 사용하여 최대한 빠른 반응 속도로 /cmd_vel 토픽 발행
- 직관적인 방향 제어: 복잡한 수치 입력 대신 일상적인 언어로 로봇의 전진, 후진, 좌/우 회전 동작을 수행
